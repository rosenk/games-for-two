...
Push `main` to `origin`. Then ensure a remote named `github` points to `https://github.com/rosenk/tic-tac-toe.git`. Fetch `github`. If `github/main` contains commits that are not in local `main`, stop and ask the user instead of force-pushing. Otherwise, push the same `main` commit to `github/main` so GitHub Pages receives the shipped version.
